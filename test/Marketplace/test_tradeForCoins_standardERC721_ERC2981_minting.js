...
contract("NFTMarketplace", accounts => {
    describe("Parameterized test for tradeForCoins with ERC721RoyaltiesMock - Balances", async () => {
        const testData = [
            // Decimals
            {
                input: {
        ...
        let market, erc721RoyaltiesMock
        ...
        beforeEach("Deploying contracts and setting up wallets", async () => {
            ...
            // Info
            decimals = await erc721RoyaltiesMock.DECIMALS()
        ...
        testData.forEach(async (dataSet) => {
            ...
            it(`Should see PnL for seller: ${sellerPnl} ETH, buyer #1: ${buyerPnl} ETH, buyer #2: ${buyer2Pnl} ETH, marketplace: ${autenticaPnl} ETH 
                ...
                // Mint
                const mint = await erc721RoyaltiesMock.mint(seller, royaltyFee, { from: seller })
                
                // First trade
                const expandedSig = await generateSignature(
                    marketplaceSignatureTypes,
                    [market.address, erc721RoyaltiesMock.address, nftId, seller, buyer, price, token, royaltyFee, 0, marketplaceFee],
                    operator)

                const case1Approve = await erc721RoyaltiesMock.approve(market.address, nftId, { from: seller })
                const case1Trade = await market.tradeForCoins(erc721RoyaltiesMock.address, nftId, price, buyer, marketplaceFee, expandedSig, { from: buyer, value: price })
                ...
                const expandedSig2 = await generateSignature(
                    marketplaceSignatureTypes,
                    [market.address, erc721RoyaltiesMock.address, nftId, buyer, buyer2, price, token, royaltyFee, 0, marketplaceFee],
                    operator)

                const case2Approve = await erc721RoyaltiesMock.approve(market.address, nftId, { from: buyer })
                const case2Trade = await market.tradeForCoins(erc721RoyaltiesMock.address, nftId, price, buyer2, marketplaceFee, expandedSig2, { from: buyer2, value: price })
